...
@hydra.main(config_path='expr/tsrn_crnn_rnn_mlp', config_name='crnn_rnn_mlp', version_base='v1.2')
def main(configs: OmegaConf):
    ...
    input_lr = torch.randn((1, 4, 16, 64)).to(main_device)
    flops1, params1 = profile(tsrn, (input_lr,))
    input_rec = torch.randn((1, 1, 32, 128), device=configs.device)
    flops2, params2 = profile(rec_model, (input_rec,))
    print(str((flops1 + flops2) / 1000**3) + 'G', str((params1 + params2) / 1000**2) + 'M')
# str(flops/1000**3)+'G', str(params/1000**2)+'M'
    resize_to_sr = transforms.Resize((32, 128), InterpolationMode.NEAREST)
    with torch.no_grad():
        loader_name = ['easy', 'medium', 'hard']
        print(f'evaluating ...')
        # sr_model.eval()
        rec_model.eval()
        avg_acc = []
        infer_time = []
        label_file = f'{infer_dir}/a_labels.txt'
        ...
        with open(label_file, 'w') as lf:
            for idx, dl in enumerate([infer_easy_dl, infer_medium_dl, infer_hard_dl]):
                ...
                for eid, ebatch in enumerate(dl):
                    ...
                    st = time()
                    sr_imgs = tsrn(lr_imgs)
                    rec_sr_imgs = normalize_fn(sr_imgs, (hr_imgs.shape[2], hr_imgs.shape[3]))
                    rec_sr_imgs = rec_sr_imgs[:, :3, :, :].mean(dim=1, keepdim=True)
                    # lr_imgs = normalize_fn(lr_imgs, (hr_imgs.shape[2],hr_imgs.shape[3]))
                    # lr_imgs = lr_imgs[:, :3, :, :].mean(dim=1, keepdim=True)
                    logits = rec_model(rec_sr_imgs)
                    torch.cuda.synchronize()
                    et = time()
                    ...
                    img_name = f'{level}_{img_name_fmt.format(eid)}'
                    lr_file = f'{level}_{lr_fmt.format(eid)}'
                    hr_file = f'{level}_{hr_fmt.format(eid)}'
                    sr_file = f'{level}_{sr_fmt.format(eid)}'
                    save_image(lr_imgs[0, :3, :, :], f'{infer_dir}/{lr_file}')
                    save_image(sr_imgs[0, :3, :, :], f'{infer_dir}/{sr_file}')
                    save_image(hr_imgs[0, :3, :, :], f'{infer_dir}/{hr_file}')
                    ...
                    line += f'{pred_labels}\n'
                    lf.write(f'{img_name} | gt: {labels} | pred: {pred_labels}\n')

                acc= sum(acc) / len(acc)
                avg_acc.append(acc)
                print(f'epoch {0}, loader {idx}, avg_acc {acc}')
            avg_acc= _weighted_avg(avg_acc)
            fps= len(infer_time) / sum(infer_time)
            print(f'epoch {fps}, avg_acc {avg_acc} / {best_acc} ({best_acc_id})')
