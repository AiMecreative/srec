...
绪论部分主要论述选题的意义、国内外研究现状以及本文主要研究的内容、研究思路以及内容安排等~@aster@coco-text@tsrn
...
简单文字图可用Typst直接绘制，复杂的图考虑使用相应的图形绘制软件完成，以提高图形表达质量~@aster。目前Typst不支持插入PDF图像，矢量图请转为SVG格式。

插图居中排列，与上文文本之间空一行。图序图名设置为五号宋体居中，图序与图名之间空一格。

#figure(
  image("../fig/fig1-1a.svg",width:50%),
