...
*结构一致性指标（SSIM）*。SSIM主要用于衡量超分辨率任务的性能，该指标通过比较超分辨率图像$I_s$和原始高分辨率图像$I_h$间的亮度$l(I_s,I_h)$、对比度$c(I_s,I_h)$和结构$s(I_s,I_h)$，对图像间的一致性进行衡量。其主要用像素值统计量衡量图像的视觉特征，即图像像素值的均值可以反映图像的明暗程度，像素间的方差可以反映图像的对比度，而两张图像之间的协方差则可以反映像素值之间的关联程度。三个子指标的计算方式如@eq4-ssim1 所示。
