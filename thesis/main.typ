...
目前绝大多数识别器需要在大规模人造数据集或真实数据集上进行训练，其中常用的人造数据集有MJSynth~@mjsynth 和SynthText~@synthtext。在大规模数据集上进行预训练的STR模型通常有较好的语义特征，泛化能力较强，而TextZoom~@tsrn 数据集规模远小于以上两种，且数据分布存在一定差异，适合作为微调数据集。经过调研，本文识别器选取CRNN~@crnn 作为主干网络，并用该网络在TextZoom数据集上做微调训练。如@fig3-crnn 所示，实线部分为
