...
//TODO: 补充图片说明
*图像中心对齐*。由于数据集通过失焦采样真实的场景文本，LR图像中心和HR图像中心会出现一定的偏移，因此模型在编码器部分设计了空间变换网络（Spatial Transformer Network, STN）~@stn 用于对图像做空间尺度的变换，将LR图像与HR图像中心对齐。同时薄板样条插值（Thin Plate Spline, TPS）~@tps 用于
