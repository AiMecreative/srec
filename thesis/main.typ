...
*识别器*。常见的工作中可以使用任何预训练识别器作为主干，只需要微调其分类头即可在相应数据集上实现较高的准确率。预训练模型中的编码器部分是用于提取图像特征的关键部分，该部分能提取多量、浅层的细节特征，而解码器部分用于处理这些浅层特征，并逐步整合为完整的深层特征，最后分类头和激活函数将深层特征映射为几率（logits）。本文的识别器受到CRNN~@crnn 的启发，在CNN和LSTM之间加入了MLP和残差连接，同时增强了模型分类头，如@fig3-recognizer 所示。由于特征图直接由本文模型的编码器或特征增强模块得到，因此本文重新训练了识别器。在消融实验中，本文将讨论微调预训练识别器和重新训练识别器之间的差异。
