...
对于串行连接的多任务模型和并行连接的多任务模型，在模型规模、推理速度上也存在一定的差异。如@tab4-3 所示，在FPS、参数量和浮点运算数方面，同等微调策略下，并行连接的模型PSRec由于不同任务可以共用编码器特征，因此本文模型的参数量少于同等配置的TSRN+CRNN串行模型；与此同时，对于串行连接的模型架构而言，模型的结构较深，因此数据流的传播速度较慢，推理速度较慢，而对于并行连接的模型而言，多个任务的推理过程在编码器可以同时进行，推理时间少于串行连接的模型。
...
*梯度剖面损失*。超分辨率任务使用不同权重的梯度剖面损失能够给模型性能带来不同程度的影响。@tab4-5a 展示了使用和不使用梯度剖面损失以及不同权重的梯度剖面损失对模型在TextZoom测试集上的性能影响。从表中可以发现，当$lambda_2=1 times 10^(-4)$时，训练的模型性能相比于其它权重更好。
...
#figure(
  ...
  caption: [前50个epoch中不确定性加权对不同微调方式模型的影响。其中绿色线条表示超分辨率任务损失权重中的参数$sigma_s$，紫色线条表示识别任务损失权重中的参数$sigma_r$，灰色虚线表示两个参数的比值。]
)<fig4-weight>
