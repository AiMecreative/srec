...
*图像中心对齐*。由于数据集通过失焦采样真实的场景文本，LR图像中心和HR图像中心会出现一定的偏移，因此模型在编码器部分设计了空间变换网络（Spatial Transformer Network, STN）~@stn 用于对图像做空间尺度的变换，将LR图像与HR图像中心对齐。同时编码器还使用薄板样条插值（Thin Plate Spline, TPS）~@tps 扩大STN变换的灵活性。STN模块实现对图像变换的预测，增强卷积对图像剪切、旋转和仿射的鲁棒性，TPS模块使用插值的方式，通过关注不同图像控制点的变换，实现对图像除控制点以外其余部分的估计。该部分的基本流程为：先通过STN预测输入图像变换后控制点的位置，再将控制点和图像输入TPS实现图像的变换。该部分数学推导见附录。

*混合特征*。编码器的定位特殊，作为不同任务的前置特征提取模块之一，编码器的设计和训练需要多次实验测试。由第二章@fig2-rec 可知，通常的识别器包含图像特征编码器和后续的序列特征学习模块，而由第二章@fig2-sr 可知，通常的超分器同样包含图像特征编码器和后续的超分图像特征学习模块。因此本文设想通过构造一种以卷积为主的特征提取编码器，同时实现文本特征的提取和超分特征的提取。预训练阶段，编码器输入的是HR高分辨率图像，超分模块的重建任务较为简单，该阶段需要注重文本特征的提取；特征增强阶段，需要在编码器后接入特征增强模块，此时模型的输入是LR低分辨率图像，此时预训练完成的编码器会先对LR低分辨率图像进行特征提取，接着将LR低分辨率特征进行增强，输出适应预训练解码器的超分特征，该超分特征含有文本信息。因而在编码器设计部分，本文主要利用编码器实现图像文本信息的浅层提取，而超分变率特征主要由后续的特征增强模块实现。后续实验证明，只需要实验两层卷积层和一个激活函数，即可实现文本特征和超分特征的浅提取。
...
#figure(
  image("fig/srb.svg", width: 80%),
  caption: [SRB基本模块和特征增强模块]
)<fig3-srb>

在推理时，该模块的输入为低分辨率图像特征，经过编码器的处理后，特征图中含有一定的语义信息，因此该模块的目的是在保有原有语义特征的条件下，在特征空间中提取超分辨率信息。本文受TSRN~@tsrn 和TATT~@tatt 模型启发，设计了多个SRB~@tsrn 模块提取超分辨率特征和PixelShuffle进行像素重排，模块的输出依然处于特征空间。
]

SRB的基本结构如@fig3-srb 所示，特征图首先通过CNN进行特征的再次提取，接着将特征图重新排列并按照垂直的方式组合使其能够输入第一个双向门控循环单元（Bidirectional Gated Recurrent Unit, BiGRU）。接着使用残差连接，输入第二个BiGRU，从而完成一次超分信息的提取。本文使用了多个堆叠的SRB，并使用主梯度损失（Gradient Profile Loss）~@gpp 对该部分进行特征层的监督训练，目的是能尽量将LR特征恢复为HR特征。
...
*识别损失*。识别损失采用CTC损失函数~@ctc，该损失常用于文本序列预测的监督。在序列模型中，采用交叉熵损失或均方误差损失时，经常需要将输出的预测值与真实值标签对齐，这种对齐方式工作量大；除此之外，预测结果中可能存在空白字符和连续字符，空白字符可能是由图像中两个文本间的空白区域预测得到的，而连续字符则可能是图像中同一字符被多次识别，也有可能是文本中原本就存在连续字符；因此解码器的工作量较大。而CTC损失
