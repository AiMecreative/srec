...
$
cal(L)_s = lambda_1 cal(L_1) + lambda_2 cal(L)_2
$<eq3-sr>
...
//TODO: 添加StepLR引用，如果有的话，补充学习率的图
本文所实现的模型使用单张显存大小为$24$G的 NVIDIA RTX 3090 显卡完成训练和推理。批量大小设置为512组数据，一组数据包含标签、$32 times 128$ 像素大小的高分辨率图像和 $16 times 64$ 像素大小的低分辨率图像。本文在3通道图像的基础上添加了一层灰度掩码，因此整个模型的输入是$4 times 16 times 64$ 大小的低分辨率图像。整个训练过程遍历$300$次训练集。模型使用AdamW~@adamw 优化器进行参数优化，并设置$beta in [0.5,0.999]$~@tsrn。学习率更新策略采用StepLR，即每隔固定的步长，将学习率按一定比例进行衰减，模型训练时，学习率经过$30$次遍历进行一次学习率衰减，对于模型参数而言，初始学习率设置为 $5 times 10^(-4)$，学习率衰减比例设置为 $0.5$；对于可训练的损失权重而言，初始学习率设置为 $1 times 10^(-3)$，衰减比例设置为0.8，如\@所示。同时为了保证训练开始时两种损失策略对不同任务有相同的初始权重，在@eq3-avgloss 中，设置 $alpha_1=alpha_2=1.0$，在@eq3-weightedloss 中，设置 $sigma_r=sigma_s=sqrt(2)/2 approx 0.7$。超分辨率任务损失函数@eq3-sr 中的 $lambda_1=20,lambda_2=1 times 10^(-4)$~@tsrn，本章后续将对该值进行消融实验。
