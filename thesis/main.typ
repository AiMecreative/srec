...
*峰值信噪比（PSNR）*。PSNR主要用于衡量超分辨率任务的性能，该指标一般用于衡量图像经有损压缩后的重建效果。其计算方式如@eq4-psnr 所示。
...
*结构一致性指标（SSIM）*~@ssim。SSIM主要用于衡量超分辨率任务的性能，该指标通过比较超分辨率图像$I_s$和原始高分辨率图像$I_h$间的亮度$l(I_s,I_h)$、对比度$c(I_s,I_h)$和结构$s(I_s,I_h)$，对图像间的一致性进行衡量。其主要用像素值统计量衡量图像的视觉特征，即图像像素值的均值可以反映图像的明暗程度，像素间的方差可以反映图像的对比度，而两张图像之间的协方差则可以反映像素值之间的关联程度。三个子指标的计算方式如@eq4-ssim1 所示。
...
值得注意的是，衡量准确率时，模型以标签作为识别统计的基本单位。
...
// [0.3699, 0.3227, 0.3074]
// NOTE: 使用 loss-step-lr 的best-psnr对比，sidernn+mlp,3srb
#figure(
  placement: auto,
  tablex(
    auto-lines:false,
    ...
    [PSRec(Ours)], [$L_2$], [-],[-],[-],[-],[-],[-],[-],[-],
    [PSRec(Ours)], [$L_2+L_("GP")$], [*23.92*],[*20.04*],[*20.81*],[*21.71*],[0.8037],[0.5342],[0.6322],[0.6640],
    hlinex()
  ),
  kind:table,
  caption: [TextZoom数据集上三种难度测试集的PSNR/SSIM指标对比。其中$L_2$表示MSE均方误差损失，$L_("GP")$表示第三章提到的梯度剖面损失。二者的权重分别为$lambda_1=20,lambda_2=1 times 10^(-4)$。由于只比较超分辨率任务，该表格省略了PSRec模型损失函数中的识别损失$L_r=L_("CTC")$。双三次插值不需要损失函数进行训练。]
)<tab4-1>

由@tab4-1 可以看出，本文搭建的模型PSRec使用与TSRN相同的如第三章所示的超分损失 $L_s = lambda_1 L_2+ lambda_2 L_("GP")$ 进行训练后，PSRec在PSNR上的指标超过了TSRN，表明本文设计的模型对于低分辨率图像的重建效果高于TSRN。而相比于只使用 $L_s=L_2$ 损失进行训练的模型，在损失函数中添加梯度剖面损失可以增强模型的超分辨率性能。这是由于梯度剖面损失提供了图像的锐度信息，将其作为损失函数一部分可以在一定程度上强化图像的边缘特征，因此用含有该信息的损失函数训练的模型也能获得更高的PSNR。

准确率方面，本文的PSRec模型使用CTC损失进行训练，并用多任务权重调整损失比例。其中识别器分别采用不同的微调策略：（1）dense：表示重新训练最后一层分类头，（2）mlp：表示将最后一层分类头更换为多层感知机并重新训练，（3）rnn：在原有BiLSTM基础上增加BiLSTM旁路，其结构与原有BiLSTM一致，输出采用结构和旁路结构的平均值，（4）rnn-mlp：结合（2）和（3）的方式。作为对比，本文同时在TextZoom训练集上微调了CRNN模型以及TSRN与CRNN的串行连接模型。@tab4-2 展现了本文并行连接的模型PSRec与CRNN以及串行连接的TSRN+CRNN在TextZoom测试集上比较的结果，除此之外，@tab4-2 还列出了CRNN在各个测试集上进行zero-shot（即，不进行任何微调）的结果，作为比较基准。
...
#figure(
  ...
  tablex(
    ...
    [TSRN+CRNN-rnn],      [-],       [-],       [-],       [-],
    [TSRN+CRNN-rnn-mlp],  [-],       [-],       [-],       [-],
    [PSRec-dense(Ours)],  [0.7024],  [0.5539],  [0.3939],  [0.5596],
    [PSRec-mlp(Ours)],    [0.6949],  [0.5367],  [0.3902],  [0.5502],
    [PSRec-rnn(Ours)],    [0.7116],  [0.5518],  [0.3924],  [0.5619],
    [PSRec-rnn-mlp(Ours)],[0.6900],  [0.5489],  [0.3931],  [0.5532],
    hlinex()
  ),
  kind:table,
...
由@tab4-2 可以看出，不进行微调的CRNN在各个测试集上进行推理的效果远远低于微调后的效果，表明本文所使用的微调方式可以带来性能的提升。其中在原有模型的基础上增加BiLSTM旁路和多层感知机MLP的微调效果最好。对于串行方式连接的模型TSRN+CRNN，它先将输入的低分辨率图像进行超分辨率处理，得到超分辨率图像，再使用超分辨率图像对CRNN识别器进行微调。由@tab4-2 所示，这种方式在测试集的准确率高于使用CRNN进行识别的准确率，表明超分辨率模型的结果能对识别器性能带来提升。但通过串行方式连接超分器和识别器的识别准确率低于本文并行连接的模型PSRec，表明串行模型处理多任务时，不同任务的耦合度较大，识别性能受到超分辨率效果的影响较大，但使用更强的微调方式可以改善这一点。本文搭建的模型PSRec实现的是多任务并行架构，识别器与超分器共用编码器特征，不会受到超分器最后结果的影响，耦合度较低，因而识别效果高于前两种方式。
...
#figure(
  ...
  kind:table,
  caption: [串行连接的模型TSRN+CRNN和本文并行连接的模型在单张图像推理时间FPS、参数量和浮点运算数上的比较。]
)<tab4-3>

对于串行连接的多任务模型和并行连接的多任务模型，在模型规模、推理速度上也存在一些差异。如@tab4-3 所示，在FPS、参数量和浮点运算数方面，同等微调策略下，并行连接的模型PSRec由于不同任务可以共用编码器特征，因此本文模型的参数量少于同等配置的TSRN+CRNN串行模型；与此同时，对于串行连接的模型架构而言，模型的结构较深，因此数据流的传播速度较慢，而对于并行连接的模型而言，多个任务的推理过程由于共用的编码器，推理时间少于串行连接的模型。
...
本节主要对本文搭建的PSRec模型进行模块层面和训练方式层面的消融实验。在特征增强模块中，本节将讨论SRB模块的个数对图像重建效果和识别效果的影响。在训练方式中，本节将讨论超分损失的组成方式以及多任务损失权重的调整对模型在TextZoom测试集上性能的影响。

*SRB数目的影响*。特征增强模块中SRB个数对模型推理性能有一定的影响。主要的比较指标包括TextZoom测试集上体现对图像的重建能力的PSNR和SSIM，以及识别器的识别准确率。不同数量的SRB模块对整个模型性能的影响结果如@tab4-4 所示。

#figure(
  tablex(
    auto-lines:false,
    columns:(1fr,0.5fr,0.5fr,0.5fr,0.5fr,0.5fr,0.5fr,0.5fr,0.5fr,0.5fr,0.5fr,0.5fr,0.5fr,),
    rows:2em,
    stroke:1pt,
    repeat-header:true, //换页时重复表头
    header-rows:2, //表头行数
    align:center+horizon,
    hlinex(),
    [],colspanx(4)[PSNR],(),(),(),colspanx(4)[SSIM],(),(),(),colspanx(4)[ACC],(),(),(),
    hlinex(),
    [$N_("SRB")$],[E],[M],[H],[*A*],[E],[M],[H],[*A*],[E],[M],[H],[*A*],
    [0],[-],[-],[-],[-],[-],[-],[-],[-],[-],[-],[-],[-],
    [1],[-],[-],[-],[-],[-],[-],[-],[-],[-],[-],[-],[-],
    [2],[-],[-],[-],[-],[-],[-],[-],[-],[-],[-],[-],[-],
    [3],[-],[-],[-],[-],[-],[-],[-],[-],[-],[-],[-],[-],
  ...
  kind:table,
  caption: [使用不同数目的SRB（$N_("SRB")$）对PSRec模型在TextZoom测试集上性能的影响。其中E表示Easy简单测试集，M表示Medium中等测试集，H表示Hard困难测试集，A表示三个测试集的加权平均。]
)<tab4-4>

由@tab4-4 所示，在模型的增强模块中使用$3$个SRB的模型性能最好。

*梯度剖面损失*。超分辨率任务使用不同权重的梯度剖面损失能够给模型性能带来不同程度的影响。@tab4-5 展示了使用和不使用梯度剖面损失以及不同权重的梯度剖面损失对模型在TextZoom测试集上的性能影响。
...
#figure(
  tablex(
    ...
    [],colspanx(4)[PSNR],(),(),(),colspanx(4)[SSIM],(),(),(),colspanx(4)[ACC],(),(),(),
    hlinex(),
    [$lambda_2(times 10^(-4))$],[E],[M],[H],[*A*],[E],[M],[H],[*A*],[E],[M],[H],[*A*],
    [0],[-],[-],[-],[-],[-],[-],[-],[-],[-],[-],[-],[-],
    [1],[-],[-],[-],[-],[-],[-],[-],[-],[-],[-],[-],[-],
    [0.5],[-],[-],[-],[-],[-],[-],[-],[-],[-],[-],[-],[-],
    [5],[-],[-],[-],[-],[-],[-],[-],[-],[-],[-],[-],[-],
  ...
  kind:table,
  caption: [使用不同权重$lambda_2$的梯度剖面损失$L_("GP")$对模型性能的影响。其中超分损失中$L_2$的权重为固定为$20$。E表示Easy简单测试集，M表示Medium中等测试集，H表示Hard困难测试集，A表示三个测试集的加权平均。]
)<tab4-5>

*不确定性权重*。在多任务学习中，使用可学习的权重参数训练模型可以给模型带来一定的性能提升。

#figure(
  tablex(
    auto-lines:false,
    columns:(3fr,0.5fr,0.5fr,0.5fr,0.5fr,0.5fr,0.5fr,0.5fr,0.5fr,0.5fr,0.5fr,0.5fr,0.5fr,),
    rows:2em,
    stroke:1pt,
    repeat-header:true, //换页时重复表头
    header-rows:2, //表头行数
    ...
    [],colspanx(4)[PSNR],(),(),(),colspanx(4)[SSIM],(),(),(),colspanx(4)[ACC],(),(),(),
    hlinex(),
    [是否使用可学习的损失权重],[E],[M],[H],[*A*],[E],[M],[H],[*A*],[E],[M],[H],[*A*],
    hlinex(),
    [×],[-],[-],[-],[-],[-],[-],[-],[-],[-],[-],[-],[-],
    [√],[-],[-],[-],[-],[-],[-],[-],[-],[-],[-],[-],[-],
    hlinex(),
  ...
  kind:table,
  caption: [使用可学习的损失权重对模型性能的影响。其中E表示Easy简单测试集，M表示Medium中等测试集，H表示Hard困难测试集，A表示三个测试集的加权平均。]
)<tab4-6>
